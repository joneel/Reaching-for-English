...
const BACKGROUND_COLOR = 'white';//'#87ceeb';
...
export default class AudioPlayer extends React.Component{
	...
	componentDidMount() {
		...
		this.openRecordings();
		//this.deleteAllDownloadedAudio();
		this.createDownloadAudioDirectory();
	}
		...
		if(!this.props.navigation.state.params.fromRecording){
			db.transaction( tx => {
				tx.executeSql('INSERT OR REPLACE INTO lessons (userType, env, tid, lid, filename, text, path, ext) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', 
					...
					FileSystem.documentDirectory + 'audio/' + this.props.navigation.state.params.name, this.props.navigation.state.params.ext]);
				}, 
				() => {//Error happened after trying to input data into table so delete table and all mp3's, and re-create table.
				...
				() => {//Input into table was successful so download mp3.
						FileSystem.downloadAsync(this.props.navigation.state.params.path,
														FileSystem.documentDirectory + 'audio/'+ this.props.navigation.state.params.name )
	...
	async deleteAllDownloadedAudio(){
		FileSystem.deleteAsync( FileSystem.documentDirectory + 'audio', {idempotent: true} );
	}
	async createDownloadAudioDirectory(){
		FileSystem.getInfoAsync(FileSystem.documentDirectory + 'audio').then(({exists}) => {
			if(!exists){//if directory doesn't exist, then create it.
			...
			if(!this.props.navigation.state.params.fromRecording){			
				...
				db.transaction(tx => {//only delete if no other references to that mp3 path.
					tx.executeSql('SELECT * FROM lessons WHERE path = ?;', 
						...
						(_, { rows: { _array } }) => {
							if(_array.length == 0){
								FileSystem.deleteAsync( FileSystem.documentDirectory +'audio/'+ this.props.navigation.state.params.name, {idempotent: true} )
		...
		return(
			<View style={styles.mainContainer}>
				...
				<TouchableOpacity activeOpacity = { 0.8 } onPress = { this.changePlayerLayout } >
                    <Text style = { labels.style }>{this.state.expandedPlayer ? 'Hide ' : ''}Player</Text>
                </TouchableOpacity>
					...
					<View style={styles.playerContainer} onLayout = {( event ) => this.getPlayerHeight( event.nativeEvent.layout.height )}>
						<View style={styles.sliderContainer}>
							<Slider
								...
								thumbTintColor='#4272b8'
								minimumTrackTintColor='#4272b8'
							/>  
				...
				<TouchableOpacity activeOpacity = { 0.8 } onPress = { this.changeRecorderLayout } >
                    <Text style = { labels.style }>{this.state.expandedRecorder ? 'Hide ' : ''}Recorder</Text>
                </TouchableOpacity>				
				...
				<View style={{ alignItems:'center', bottom: 1, backgroundColor: BACKGROUND_COLOR }}>
					{this.props.navigation.state.params.fromRecording && <Text style={labels.style}>{'New Lesson'}</Text>}
				</View>
